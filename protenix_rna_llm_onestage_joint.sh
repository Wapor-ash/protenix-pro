#!/bin/bash
# =============================================================================
# Protenix + RNA LLM : 1-stage joint finetune
# 思路：
# 1. 以你当前稳定的 1-stage baseline 为底
# 2. 直接打开 RNA LM，从 step 0 开始 joint train
# 3. 不使用 two_stage，避免 stage handoff 带来的不稳定
# 4. 尽量少改：保留 baseline 的 lr / warmup / ema / loss / data 设置
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${SCRIPT_DIR}/output/sota_one_stage_llm_joint"
...
echo "🚀 Launching 1-stage joint finetuning with RNA LLM..."

python3 ./runner/train.py \
    --model_name "protenix_base_20250630_v1.0.0" \
    --run_name "sota_one_stage_llm_joint" \
    --seed 42 \
    --base_dir "${OUTPUT_DIR}" \
    --dtype "bf16" \
...
echo "1-stage joint finetuning with RNA LLM complete!"
