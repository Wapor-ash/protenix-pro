#!/bin/bash
# =============================================================================
# Protenix + RNA LLM : 1-stage joint finetune — BOTH injection mode
# 思路：
# 1. 以稳定的 1-stage baseline 为底
# 2. RNA LLM embedding 同时在两处注入：
#    a) InputFeatureEmbedder (加到 s_inputs，模仿 ESM)
#    b) DiffusionConditioning (加到 s_trunk，和 single repr 融合)
# 3. 两处均使用 Zero-init projection: 训练开始时模型行为与不使用 LLM 完全一致
# 4. 1-stage joint training，不使用 two_stage
# 5. injection_mode="both" — 同时启用两种注入
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${SCRIPT_DIR}/output/sota_one_stage_llm_both_inject"
...
echo "🚀 Launching 1-stage joint finetuning with RNA LLM (BOTH injection: input + diffusion)..."

python3 ./runner/train.py \
    --model_name "protenix_base_20250630_v1.0.0" \
    --run_name "sota_one_stage_llm_both_inject" \
    --seed 42 \
    --base_dir "${OUTPUT_DIR}" \
    --dtype "bf16" \
...
echo "1-stage joint finetuning with RNA LLM (BOTH injection) complete!"
