#!/bin/bash
# =============================================================================
# Protenix Fine-tune: RNA-only LLM Embeddings (AIDO)
#
# Uses only RNA embeddings (2048-dim), DNA embeddings disabled.
# DNA tokens receive zero vectors. All key embedding parameters are
# configurable via command-line arguments.
#
# Usage:
#   bash finetune/finetune_rna_only.sh                          # defaults: input injection
#   bash finetune/finetune_rna_only.sh --injection_mode diffusion
#   bash finetune/finetune_rna_only.sh --injection_mode both --gate_mode scalar
#   bash finetune/finetune_rna_only.sh --two_stage true --max_steps 50000
#   bash finetune/finetune_rna_only.sh --max_steps 50000 --lr 0.0005
# =============================================================================
set -euo pipefail
...
USE_WANDB="true"
RUN_NAME="0311_16_rna_input"                     # auto-generated if empty
# ===================================================================
...
# AIDO RNA Embedding paths (DNA disabled)
...
DNA_EMBEDDING_DIM=1024  # Not used but required by config schema
MODEL_NAME="aido"

# Auto-generate run name if not set
if [ -z "${RUN_NAME}" ]; then
    RUN_NAME="ft_rna_only_${INJECTION_MODE}"
...
# ===================== Banner =====================
echo "========================================================"
echo "  Protenix Fine-tune: RNA-only (AIDO)"
echo "  RNA: ${RNA_EMBEDDING_DIM}-dim  |  DNA: DISABLED"
...
echo ""
echo "Fine-tune complete: RNA-only | injection=${INJECTION_MODE} | gate=${GATE_MODE}"
